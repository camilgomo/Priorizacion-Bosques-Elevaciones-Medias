...
Verm_chry_AbundCRPan.ele[Verm_chry_AbundCRPan.ele==0] <- NA

plot(Verm_chry_AbundCRPan.ele)

## transformamos los valores de abundancia en un rango de 0 a 1 normalizando

min_val <- min(Verm_chry_AbundCRPan.ele[], na.rm = T)
max_val <- max(Verm_chry_AbundCRPan.ele[], na.rm = T)

Vchry = (Verm_chry_AbundCRPan.ele - min_val)/(max_val - min_val)
plot(Vchry)
hist(Vchry, ylim=c(0,4000))

## calculamos los cuantiles para determinar un umbral de abundancia para 
##priorizar la presencia y abundancia de la especie
quantile(Vchry, c(.50, .75))

#      50%       75% 
# 0.2540761 0.4531441

Vchry50 <- reclassify(Vchry, c(-Inf, 0.2540761, 0, 0.2540761, Inf, 1))
plot(Vchry50)

Vchry75 <- reclassify(Vchry, c(-Inf, 0.4531441, 0, 0.4531441, Inf, 1))
plot(Vchry75) 

## Guardar para usos futuros, capa transformada
writeRaster(Vchry, "Vchry_invierno.tif", format = "GTiff")
...
Car_ca_AbundCRPan.ele[Car_ca_AbundCRPan.ele==0] <- NA

plot(Car_ca_AbundCRPan.ele)

## transformamos los valores de abundancia en un rango de 0 a 1 normalizando

min_val <- min(Car_ca_AbundCRPan.ele[], na.rm = T)
max_val <- max(Car_ca_AbundCRPan.ele[], na.rm = T)

Cca = (Car_ca_AbundCRPan.ele - min_val)/(max_val - min_val)
plot(Cca)
hist(Cca, ylim=c(0,4000))

quantile(Cca, c(.50, .75))

## 50%       75% 
## 0.1639664 0.2667365

Cca50 <- reclassify(Cca, c(-Inf, 0.1639664, 0, 0.1639664, Inf, 1))
plot(Cca50)

Cca75 <- reclassify(Cca, c(-Inf, 0.2667365, 0, 0.2667365, Inf, 1))
plot(Cca75)

## Guardar para usos futuros, capa transformada
writeRaster(Cca, "Cca_migracion.tif", format = "GTiff")
...
Set_ce_AbundCRPan.ele[Set_ce_AbundCRPan.ele==0] <- NA

plot(Set_ce_AbundCRPan.ele)

## transformamos los valores de abundancia en un rango de 0 a 1 normalizando

min_val <- min(Set_ce_AbundCRPan.ele[], na.rm = T)
max_val <- max(Set_ce_AbundCRPan.ele[], na.rm = T)

## Estandarizamos de 0 a 1 normalizando

Sce = (Set_ce_AbundCRPan.ele - min_val)/(max_val - min_val)
plot(Sce)
hist(Sce, ylim=c(0,6000))

quantile(Sce, c(.50, .75))

## 50%        75% 
## 0.04207213 0.09445563 

Sce50 <- reclassify(Sce, c(-Inf, 0.04207213, 0, 0.04207213, Inf, 1))
plot(Sce50)

Sce75 <- reclassify(Sce, c(-Inf, 0.09445563, 0, 0.09445563, Inf, 1))
plot(Sce75) 

## Guardar para usos futuros, capa transformada
writeRaster(Sce, "Sce_migracion.tif", format = "GTiff")
plot(Sce)
Cca_migracion=raster("Cca_migracion.tif")
...
writeRaster(FOCAL_Aves_Elev50, "Focal_Areas 2 sp50.tif", format = "GTiff", 
            overwrite=TRUE)
...
writeRaster(FOCAL_Aves_Elev75, "Focal_Areas 2 sp75.tif", format = "GTiff", 
            overwrite=TRUE)
